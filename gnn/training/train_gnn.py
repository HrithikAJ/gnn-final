"""
Training pipeline for GNN attack risk forecasting.
Handles CUDA detection, reproducible seeding, class imbalance handling,
early stopping, best-model checkpointing, and metric logging.
"""
import os
import sys
import random
...
def evaluate_model(
    ...
    """Evaluate model on a DataLoader and return loss, metrics, true labels, and probabilities."""
    model.eval()
    total_loss = 0.0
    total_samples = 0
...
def train_gnn(
    ...
    """
    Train and evaluate AttackRiskGNN with early stopping and checkpointing.
    """
    ...
    for g in graphs:
        ...
        if feature_mode == "structure_only":
            # Node features set to constant 1.0 (degree/topology only)
            g_copy.x = torch.ones((g_copy.num_nodes, 1), dtype=torch.float32)
        elif feature_mode == "node_only":
            # No edge features, just node features
            g_copy.edge_attr = torch.zeros((g_copy.edge_index.shape[1], 0), dtype=torch.float32)
    ...
    # Initialize model
    in_channels = processed_graphs[0].x.shape[1]
...
def train_temporal_forecaster(
    ...
    config: Dict[str, Any],
    device: Optional[torch.device] = None,
    pretrained_gnn: Optional[AttackRiskGNN] = None,
) -> Dict[str, Any]:
    """
    ...
    # Use pretrained GNN encoder or train one
    if pretrained_gnn is not None:
        gnn_encoder = pretrained_gnn
    else:
        gnn_encoder = AttackRiskGNN(
            in_channels=in_channels,
            hidden_channels=model_cfg.get("hidden_channels", 64),
            num_layers=model_cfg.get("num_layers", 2),
            dropout=model_cfg.get("dropout", 0.3),
    ...
    with torch.no_grad():
        for g in graphs:
            ...
            emb = gnn_encoder.get_graph_embedding(g_dev.x, g_dev.edge_index, batch_vec)
            embeddings.append(emb.cpu().squeeze(0))  # (D,)
    ...
    # Valid indices start at window_size - 1
    valid_indices = list(range(window_size - 1, len(graphs)))
    X_seq = []
    y_seq = []
    for vi in valid_indices:
        start_idx = vi - window_size + 1
        seq = stacked_embeddings[start_idx : vi + 1]  # (W, D)
        X_seq.append(seq)
        y_seq.append(labels[vi])

    X_seq = torch.stack(X_seq, dim=0)  # (N_samples, W, D)
